 ...
 */
@Controller
public class GetPacketListController extends BaseController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping({"/GetPacketListController.indexPage"})
	   public ModelAndView indexPage(HttpServletRequest request, HttpServletResponse response)
	    ...
	    ModelAndView view = new ModelAndView("/getPacketList.jsp", this.viewData);
	    //获取 发的红包
	    info.put("page", 0);
	    info.put("rows", 100);
	    info.put("userId", this.getUser().getUserId());
	    List<Map> sendPacketMaps = sendPacketServiceImpl.getSendPacketListByZOpenId(info);
