 ...
 */
package com.java110.controller.weixin.auth;
 ...
 */
@Controller
public class WLoginAuthController extends BaseController {
	...
	@RequestMapping( { "/WLoginAuthController.indexPage" })
	public ModelAndView indexPage(HttpServletRequest request,
		...
		String url = Global.GET_ACCESS_TOKEN_URL.replace("APPID", Global.APPID)
			...
			if (null != jsonObject) {
				if(jsonObject.containsKey("access_token")) {
					...
					if (userMap == null) {
						//拉去用户信息
						 url = Global.GET_USER_INFO_SNSURL.replace("ACCESS_TOKEN", authInfo.access_token)
						...
						paramIn
								.put("name", wechatUserInfo.nickname);
						paramIn.put("sex", wechatUserInfo.sex);
						paramIn.put("email", "");
						paramIn.put("phone", "");
						paramIn.put("wOpenId", wechatUserInfo.openid);
						...
						if (saveUserFlag > 0) {
							...
							if (accountFlag > 0) {
								...
								user.setName(wechatUserInfo.nickname);
								user.setSex(String.valueOf(wechatUserInfo.sex));
								user.setEmail("");
								user.setPhone("");
								user.setzOpenId(wechatUserInfo.openid);
							}
						}
					} else {
						// 将用户保存至session中
